  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-8">
      <div className="text-center max-w-2xl mx-auto animate-fade-in">
        <h1 className="font-serif text-4xl md:text-6xl font-bold mb-4 text-noir-gold">
          Clue Craft
        </h1>
        <p className="text-xl mb-6 text-noir-paper/90">
          Test your detective skills with AI-generated crime scenarios
        </p>
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          <div className="flex items-center">
            <Brain className="h-5 w-5 mr-2 text-noir-gold" />
            <span>Sharpen Analytical Skills</span>
          </div>
          <div className="flex items-center">
            <ScrollText className="h-5 w-5 mr-2 text-noir-gold" />
            <span>Unique Cases Every Time</span>
          </div>
          <div className="flex items-center">
            <Users className="h-5 w-5 mr-2 text-noir-gold" />
      ...
      <div className="w-full max-w-4xl">
        <h2 className="font-serif text-2xl font-bold mb-4 text-center">Select Difficulty Level</h2>
            ...
            <Card 
              ...
              className={`p-6 cursor-pointer transition-all duration-200 hover:shadow-lg border-2 ${
                gameState.difficulty === option.level 
                  ? 'border-noir-gold bg-noir-dark/80' 
                  : 'border-transparent bg-noir-dark/50'
              }`}
              onClick={() => setDifficulty(option.level)}
            >
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-serif text-xl font-bold">{option.name}</h3>
                <Badge className={`${option.color}`}>{option.level}</Badge>
              </div>
              <p className="text-sm text-noir-paper/70 mb-4">{option.description}</p>
            </Card>
          ))}
        </div>
      </div>
      ...
      <Button 
        className="bg-noir-gold hover:bg-noir-gold/80 text-noir-dark font-bold py-3 px-8 text-lg"
